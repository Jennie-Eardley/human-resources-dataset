...
**Recruitment**
 ...
 The data covers the time period of 2006 to 2017. This graph shows the dates that hires were made. The month with the highest number of new recruits was in September of January with 15 new recruits. 
```{r echo = FALSE}
...
```
If we combine the years together, we can also see that there are some seasonal differences in hiring. January is clearly the most popular month for hiring. This is perhaps unsurprising, compensating for the low hire rate in December. It is interesting that hiring levels in April are relatively low, this is often an optimum month for hiring as budgets are agreed for the new financial year. 
```{r echo = FALSE}
indexed_monthly_tsibble <- hr_tsibble %>% 
  index_by(monthly_hire = ~ month(., label = TRUE)) %>% 
  summarise(seasonal_count = n())
