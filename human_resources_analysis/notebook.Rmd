...
```
```{r}
forecasting_numeric <- hr %>% 
  select( dept_id, perf_score_id, pay_rate, position_id, engagement_survey, emp_satisfaction)
```
```{r}
forecasting_non_numeric <- hr %>% 
  select(sex, race_desc)
```
